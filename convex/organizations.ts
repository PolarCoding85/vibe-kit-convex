...
export const upsertFromClerk = internalMutation({
  ...
  async handler(ctx, { data }) {
    ...
    if (org === null) {
      return await ctx.db.insert('organizations', orgAttributes)
    } else {
...
export const deleteFromClerk = internalMutation({
  ...
  handler: async (ctx, { id }) => {
    ...
    if (!organization) {
      console.warn(`Cannot delete organization: Organization with external ID ${id} not found`);
      return;
    }
    ...
    console.log(`Successfully deleted organization ${id} and ${memberships.length} memberships`);
    return { success: true };
  }
})
...
async function organizationByExternalId(ctx: QueryCtx | MutationCtx, externalId: string) {
  return await ctx.db
    .query('organizations')
